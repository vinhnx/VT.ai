...
def check_mkdocs_installation():
	...
	if not mkdocs_version:
		...
		if install == "y":
			run_command("uv pip install -U mkdocs mkdocs-material mkdocstrings mkdocstrings-python", 
...
def deploy_docs():
	...
	run_command("mkdocs gh-deploy --force", "Deploying documentation")
	return True
...
def main():
	...
	if update_docs == "y":
		...
		if docs_built:
			...
			deploy_docs_choice = input("Deploy documentation to GitHub Pages? (y/n): ").lower()

...
def get_tier_from_subscription(subscription: stripe.Subscription) -> str:
    ...
    if (
        price_id == "price_pro_tier_id_from_stripe"
    ):  # Replace with actual Stripe Price ID
        return "pro"
    elif (
        price_id == "price_enterprise_tier_id_from_stripe"
    ):  # Replace with actual Stripe Price ID
        return "enterprise"
    else:
        ...
        if subscription.status == "active":
            logger.warning(
                f"Unknown Price ID {price_id} for active subscription {subscription.id}. Defaulting tier."
            )
...
def update_user_subscription_status(
    ...
    try:
        ...
        if response.data:
            logger.info(
                f"Successfully updated subscription for Stripe customer {stripe_customer_id} to tier {tier}, status {subscription_status}."
            )
        else:
            logger.error(
                f"Failed to update subscription for Stripe customer {stripe_customer_id}. Response: {response.error or 'No data returned'}"
            )
    except Exception as e:
        logger.error(
            f"Error updating Supabase for Stripe customer {stripe_customer_id}: {e}",
            exc_info=True,
        )
...
async def stripe_webhook(request: Request, stripe_signature: str | None = Header(None)):
    ...
    if stripe_webhook_secret and stripe_signature:
        ...
        logger.warning(
            "Stripe webhook secret is configured, but no signature was provided with the request."
        )
        ...
        try:
            ...
            logger.warning(
                f"Stripe webhook event received (NO SIGNATURE VERIFICATION): {event.type}"
            )
        except Exception as e:
            logger.error(
                f"Error constructing event from payload (no verification): {e}"
            )
            raise HTTPException(
                status_code=400, detail="Invalid payload or event construction error."
            )
    ...
    if event.type == "invoice.paid":
        ...
        if (
            ...
            or invoice.billing_reason == "subscription_cycle"
        ):
            ...
            if stripe_customer_id and subscription_id:
                try:
                    ...
                    update_user_subscription_status(
                        stripe_customer_id, tier, "active", subscription_id
                    )
                    logger.info(
                        f"Processed invoice.paid for customer {stripe_customer_id}, subscription {subscription_id}, tier {tier}."
                    )
                    ...
                    logger.error(
                        f"Error processing invoice.paid for {stripe_customer_id}: {e}",
                        exc_info=True,
                    )
            else:
                logger.warning(
                    f"Invoice.paid event {invoice.id} missing customer or subscription ID."
                )
        ...
        if stripe_customer_id:
            tier = get_tier_from_subscription(subscription)
            update_user_subscription_status(
                stripe_customer_id, tier, subscription.status, subscription.id
            )
            logger.info(
                f"Processed customer.subscription.updated for customer {stripe_customer_id}, status {subscription.status}, tier {tier}."
            )
        else:
            logger.warning(
                f"customer.subscription.updated event {subscription.id} missing customer ID."
            )
        ...
        if stripe_customer_id:
            ...
            update_user_subscription_status(
                stripe_customer_id, "free", "canceled", subscription.id
            )
            logger.info(
                f"Processed customer.subscription.deleted for customer {stripe_customer_id}. User set to free tier."
            )
        else:
            logger.warning(
                f"customer.subscription.deleted event {subscription.id} missing customer ID."
            )

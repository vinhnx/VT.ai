...
def test_request_history():
    ...
    if history:
        logger.info("✅ Request History for %s (%d requests):", test_user_id, len(history))
        for i, request in enumerate(history[:3], 1):  # Show first 3
            logger.info("  %d. Model: %s, Status: %s, Tokens: %s, Time: %s", 
                       i, request.get("model"), request.get("status"), 
                       request.get("tokens_used"), request.get("request_time"))
    else:
...
def test_token_breakdown():
    ...
    if breakdown:
        ...
        for item in breakdown:
            logger.info("  Model: %s (%s) - Requests: %s, Tokens: %s, Cost: $%s", 
                       item.get("model"), item.get("provider"), 
                       item.get("request_count"), item.get("total_tokens"),
                       item.get("total_cost"))
    else:
...
def test_recent_activity():
    ...
    if activity:
        ...
        for i, request in enumerate(activity[:3], 1):  # Show first 3
            logger.info("  %d. User: %s, Model: %s, Status: %s, Time: %s", 
                       i, request.get("email") or request.get("user_id") or "Anonymous", 
                       request.get("model"), request.get("status"), 

...
def example_openai_chat():
    ...
    try:
        ...
        logger.info(
            "Tokens used: %s", response.usage.total_tokens if response.usage else "N/A"
        )
    ...
    try:
        response = quick_completion(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "What's a fascinating fact about machine learning?",
                }
            ],
            user="user_123",
            max_tokens=50,
...
def example_image_generation():
    ...
    try:
        ...
        logger.info(
            ...
            response.data[0].url if response.data else "No URL",
        )
        return response
...
def demonstrate_token_tracking():
    ...
    for model in models_to_test:
        try:
            response = quick_completion(
                model=model,
                messages=[{"role": "user", "content": f"Say hello using {model}"}],
                user=user_id,
                max_tokens=20,
            )
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(
        "Demo completed! Check your Supabase request_logs table to see logged requests."
    )
